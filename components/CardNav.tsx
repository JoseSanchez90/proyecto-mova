...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`card-nav-content absolute left-0 right-0 top-[60px] bottom-0 p-6 flex flex-col items-stretch gap-2 justify-start z-1 ${
            isExpanded
  ...
  bg-black dark:bg-gray-800 text-white dark:text-white transition-colors duration-300`}
              ref={setCardRef(idx)}
            >
              <div className="nav-card-label tracking-[-0.5px] text-xl font-bold">
                {item.label}
              </div>
              <div className="nav-card-links mt-auto flex flex-col gap-0.5">
                {item.links?.map((lnk, i) => (
                  ...
                  >
                    <GoArrowUpRight
                      className="nav-card-link-icon shrink-0 w-5 h-5"
                      aria-hidden="true"
                    />
                    {lnk.label}
                  </a>
